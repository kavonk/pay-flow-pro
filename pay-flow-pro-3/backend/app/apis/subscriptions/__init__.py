...
class FeatureAccessResponse(BaseModel):
    """Defines the detailed feature access rights for a user."""
    plan_name: str
    invoice_limit: int
    team_member_limit: int
    dunning_rules_limit: int
    recurring_billing: bool
    bulk_invoice_sending: bool
    custom_email_templates: bool
    custom_branding: bool
    pdf_customization: bool
    custom_domain: bool
    api_access: str  # e.g., "none", "read-only", "full"
    webhooks: bool
    advanced_analytics: bool
    customer_insights: bool
    revenue_reports: bool
    advanced_integrations: bool
    white_label: bool
    team_collaboration_tools: bool
    role_based_permissions: bool
...
async def get_subscription_plans() -> List[PublicSubscriptionPlanResponse]:
...
async def start_trial(request: StartTrialRequest, user: AuthorizedUser) -> UserSubscriptionResponse:
    ...
    # Get trial plan based on request
    trial_plan = await repo.get_subscription_plan_by_slug(request.plan_slug)
    if not trial_plan:
        raise HTTPException(status_code=500, detail=f"Plan '{request.plan_slug}' not found")
    
    stripe_customer_id = request.stripe_customer_id
    
    # If payment method ID is provided, create or attach to Stripe customer
    if request.payment_method_id:
        try:
            ...
            print(f"Created Stripe customer {stripe_customer_id} with payment method {request.payment_method_id}")
            
        except Exception as e:
            print(f"Error creating Stripe customer: {e}")
            # Don't fail the trial creation if Stripe customer creation fails
            # This allows users to still get a trial and upgrade later
            print(f"Continuing with trial creation without Stripe customer")
...
async def create_customer_portal(request: CustomerPortalRequest, user: AuthorizedUser) -> CustomerPortalResponse:
    ...
    if not stripe_customer_id:
        try:
            ...
            print(f"Successfully created Stripe customer {stripe_customer_id} and updated subscription {subscription.id}")
            
        except stripe.error.StripeError as e:
            raise HTTPException(status_code=400, detail=f"Stripe error creating customer: {str(e)}")
        except Exception as e:
...
async def create_customer_portal(request: CustomerPortalRequest, user: AuthorizedUser) -> CustomerPortalResponse:
    ...
    try:
        ...
        current_subscription = await repo.get_user_subscription()
        if not current_subscription:
            # This should ideally not happen if trials are auto-created, but as a fallback:
            raise HTTPException(status_code=404, detail="No active subscription found. Please start a trial first.")

        stripe_customer_id = current_subscription.stripe_customer_id

        # If no Stripe customer ID exists, create one
        if not stripe_customer_id:
            print(f"User {user.sub} does not have a Stripe customer ID. Creating one now.")
            try:
                # Extract user email safely
                user_email = getattr(user, 'email', None) or getattr(user, 'primaryEmail', None)
                ...
                customer = stripe.Customer.create(
                    ...
                    metadata={
                        "user_id": user.sub,
                        "account_id": str(current_subscription.account_id)
                    }
                )
                stripe_customer_id = customer.id
                
                # Save the new customer ID back to the subscription record
                await repo.update_stripe_customer_id(current_subscription.id, stripe_customer_id)
                print(f"Successfully created Stripe customer {stripe_customer_id} for user {user.sub} and saved to subscription.")

            except stripe.error.StripeError as e:
                print(f"Error creating Stripe customer for user {user.sub}: {e}")
                raise HTTPException(status_code=500, detail=f"Could not create a billing profile: {e}")
            except Exception as e:
                print(f"A general error occurred during Stripe customer creation for user {user.sub}: {e}")
                raise HTTPException(status_code=500, detail="Could not set up your billing profile.")

        # Now, create the customer portal session
        session = stripe.billing_portal.Session.create(
            customer=stripe_customer_id,
            return_url=request.return_url,
        )
        ...
        print(f"Stripe error creating customer portal for user {user.sub}: {str(e)}")
        # Check for common, user-facing errors
        if "No such customer" in str(e):
             raise HTTPException(
                status_code=400, 
                detail="Your billing profile could not be found. Please contact support."
            )
        ...
        print(f"Error creating customer portal for user {user.sub}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
