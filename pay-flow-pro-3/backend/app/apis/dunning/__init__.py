...
from app.apis.subscription import get_subscription_feature_access
...
async def create_dunning_rule(rule: DunningRule, user = Depends(FeatureGuard("dunning_rules"))):
    conn = await get_db_connection()
    try:
        account_id = await get_user_account_id(user.sub, conn)
