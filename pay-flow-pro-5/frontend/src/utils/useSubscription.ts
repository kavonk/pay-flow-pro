...
export const subscriptionQueryKeys = {
  ...
  featureAccess: (userId?: string) => ['subscription', 'feature-access', userId] as const,
};

// Hook for current user subscription with caching
export const useCurrentSubscription = (userId?: string) => {
  return useQuery({
    queryKey: subscriptionQueryKeys.current(userId),
...
export const useFeatureAccess = (userId?: string) => {
  return useQuery({
    queryKey: subscriptionQueryKeys.featureAccess(userId),
...
export const useSubscription = (userId?: string) => {
  ...
  } = useFeatureAccess(userId);

  const queryClient = useQueryClient();

  // Mutation for starting trial
  const startTrialMutation = useMutation({
    mutationFn: async (data: { plan_slug: string; cardData?: { customer_id?: string; last_four?: string; brand?: string } }) => {
      const { plan_slug, cardData } = data;
