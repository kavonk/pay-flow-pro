...
const InvoiceActionsDrawer: React.FC<InvoiceActionsDrawerProps> = ({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      // Generate a simple PDF-like content for download
      ...
      link.href = url;
      link.download = `invoice-${invoice.invoice_number || invoice.id.slice(0, 8)}.txt`;
  ...
  // Generate simple PDF-like content for download
  const generateInvoicePDFContent = (invoice: InvoiceResponse) => {
    const companyName = brandingSettings?.company_name || 'PayFlow Pro';
    const businessEmail = brandingSettings?.business_email || 'hello@payflowpro.com';
    const businessPhone = brandingSettings?.business_phone || '';
...
PAYMENT INFORMATION:
----------------------------------------------
Pay online: ${invoice.stripe_payment_link_url || 'Payment link not available'}

${brandingSettings?.company_name ? `Thank you for choosing ${brandingSettings.company_name}!` : 'Thank you for your business!'}
