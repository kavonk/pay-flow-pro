...
import { useFinancialStats, useSettlementSummary, useUserRole, usePayoutAccount } from "utils/queryHooks";
...
export default function DashboardPage() {
  ...
  const user = useUser();
  const { data: stats, isLoading: statsLoading, error: statsError } = useFinancialStats(user?.id);
            ...
            {settlementSummary && (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                <StatCard
                  ...
                  value={formatCurrency(Number(settlementSummary.stripe_fees))}
                  description="Stripe processing fees"
                  icon={CreditCard}
                />
                <StatCard
                  title="Platform Fees"
                  value={formatCurrency(Number(settlementSummary.platform_fees))}
                  description="PayFlow Pro platform fees"
                  icon={Wallet}
                />
                <StatCard
                  title="Net Payout"
