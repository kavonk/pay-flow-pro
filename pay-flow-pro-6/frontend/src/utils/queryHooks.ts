...
export const queryKeys = {
  financialStats: (userId?: string) => ['financial-stats', userId] as const,
  settlementSummary: (userId?: string) => ['settlement-summary', userId] as const,
...
export const teamQueryKeys = {
  all: ['team'] as const,
  members: (userId?: string) => ['team', 'members', userId] as const,
  invitations: (userId?: string) => ['team', 'invitations', userId] as const,
  role: (userId?: string) => ['team', 'role', userId] as const,
  invitationDetails: (token: string) => ['invitation-details', token] as const,
};

// Financial Stats Hook
export const useFinancialStats = (userId?: string) => {
  return useQuery({
    queryKey: queryKeys.financialStats(userId),
    ...
    placeholderData: (previousData) => previousData, // Keep previous data to prevent flicker
    retry: 2,
  });
}
...
export const useSettlementSummary = (userId?: string) => {
  return useQuery({
    queryKey: queryKeys.settlementSummary(userId),
...
export const useTeamMembers = (userId?: string) => {
  return useQuery({
    queryKey: teamQueryKeys.members(userId),
    queryFn: async () => {
...
export const useTeamInvitations = (userId?: string) => {
  return useQuery({
    queryKey: teamQueryKeys.invitations(userId),
    queryFn: async () => {
...
export const useUserRole = (userId?: string) => {
  return useQuery({
    queryKey: teamQueryKeys.role(userId),
    queryFn: async () => {
...
export const useTeamData = (userId?: string) => {
  const members = useTeamMembers(userId);
  const invitations = useTeamInvitations(userId);
