...
async def process_payment_success(payment_data: Dict[str, Any]):
    ...
    try:
        ...
        user_id = metadata.get('user_id') # Fallback
        
        stripe_customer_id = payment_data.get('customer')

        if not invoice_id:
            print("Missing invoice_id in payment metadata")
            return

        if not user_id and not stripe_customer_id:
            print("Missing user_id and stripe_customer_id in payment metadata")
            return

        repo = None
        # Prioritize stripe_customer_id for user lookup
        if stripe_customer_id:
            # Need a temporary repo to perform the lookup
            temp_repo = PaymentRepository(user_id="system_user")
            user_id_from_mapping = await temp_repo.get_user_by_stripe_customer_id(stripe_customer_id)
            if user_id_from_mapping:
                user_id = user_id_from_mapping
                repo = PaymentRepository(user_id)
            elif user_id:
                # Fallback to user_id from metadata if mapping not found
                repo = PaymentRepository(user_id)
        elif user_id:
            repo = PaymentRepository(user_id)

        if not repo:
            print(f"Could not establish repository for payment processing. Stripe Customer ID: {stripe_customer_id}")
            return
        ...
        if not invoice:
            print(f"Invoice {invoice_id} not found for user {user_id}")
            return
